...
function ImageSlider() {
  ...
  const nextSlide = () => {
    setCurrentSlide(currentSlide === length-1 ? 0 : current+1);
  };
  ...
  return (
    <section className="flex flex-row justify-center items-center p-40 static border-red-600">
      <ChevronDoubleLeftIcon className="w-10 h-10 absolute left-10 top-[500px] cursor-pointer z-10" />

      {image.map((imgData, index) => {
        return <img key={index} alt={imgData.title} src={imgData.url} className="w-100 h-100 rounded" />;
      }
      )}
      <ChevronDoubleRightIcon className="w-10 h-10 absolute right-10 top-[500px] cursor-pointer z-10" />
