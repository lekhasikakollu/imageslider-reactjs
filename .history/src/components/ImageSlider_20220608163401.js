...
function ImageSlider() {
  ...
  return (
    <section className="bg-violet-50" onClick={prevSlide}>
    <ChevronDoubleRightIcon className="w-5 h-5"></ChevronDoubleRightIcon>
      
      {image.map((imgData, index) => {
        return <img key={index} alt={imgData.title} src={imgData.url} />;
        }
       )}
       <ChevronDoubleRightIcon className="w-5 h-5" onClick={nextSlide}></ChevronDoubleRightIcon>
